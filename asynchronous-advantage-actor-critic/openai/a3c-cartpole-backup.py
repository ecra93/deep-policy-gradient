...
class Master(object):
    ...
    def initialize_architecture(self):
        ...
        with tf.name_scope("flat-layer-1"):
            self.flat_W = weight_variable([NUM_STATES, 16])
            self.flat_b = bias_variable([16])
            self.flat = tf.matmul(self.s, self.flat_W) + self.flat_b
            self.flat = tf.nn.relu(self.flat)

        with tf.name_scope("policy"):
            self.policy_W = weight_variable([16,NUM_ACTIONS])
            self.policy_b = bias_variable([NUM_ACTIONS])
            self.policy = tf.matmul(self.flat, self.policy_W) + self.policy_b
            self.policy = tf.nn.softmax(self.policy)

        with tf.name_scope("value"):
            self.value_W = weight_variable([16,1])
    ...
    def train(self):
        ...
        if sess.run(self.global_step) % 100 == 0:
            self.save_network()
...
if __name__ == "__main__":
    ...
    Drone(master, sess).run()

    workers = [Drone(master, sess) for i in range(NUM_DRONES)]
