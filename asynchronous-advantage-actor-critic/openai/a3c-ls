...
class Master(object):
    ...
    def initialize_architecture(self):
        ...
        with tf.name_scope("recurrent-layer"):
            self.lstm_cell = tf.contrib.rnn.BasicLSTMCell(NUM_STATES,
                forget_bias=1)
            self.s_seq = tf.expand_dims(self.s, axis=1)
            initial_state = self.lstm_cell.zero_state(batch_size=1,
                dtype=tf.float32)
            self.lstm, _ = tf.nn.dynamic_rnn(self.lstm_cell,
                self.s_seq, initial_state=initial_state, time_major=True)
            self.lstm_flat = tf.reshape(self.lstm, [-1, NUM_STATES])
        ...
        with tf.name_scope("flat-layer-1"):
            ...
            self.flat = tf.matmul(self.lstm_flat, self.flat_W) + self.flat_b
